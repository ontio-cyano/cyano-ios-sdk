...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"ONTOMBProgressHUD needs to be accessed on the main thread.");
...
@interface ONTOMBProgressHUDRoundedButton : UIButton
@end


@implementation ONTOMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ONTOMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ONTOMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (ONTOMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            ONTOMBProgressHUD *hud = (ONTOMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = ONTOMBProgressHUDAnimationFade;
    _mode = ONTOMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(ONTOMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == ONTOMBProgressHUDAnimationZoom) {
        type = animatingIn ? ONTOMBProgressHUDAnimationZoomIn : ONTOMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == ONTOMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == ONTOMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == ONTOMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == ONTOMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    ONTOMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<ONTOMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    backgroundView.style = ONTOMBProgressHUDBackgroundStyleSolidColor;
    ...
    UIButton *button = [ONTOMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    ONTOMBProgressHUDMode mode = self.mode;
    if (mode == ONTOMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == ONTOMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ONTOMBProgressHUDModeDeterminate || mode == ONTOMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (mode == ONTOMBProgressHUDModeAnnularDeterminate) {
            [(MBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == ONTOMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == ONTOMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[ONTOMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ONTOMBProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBRoundProgressView appearanceWhenContainedIn:[ONTOMBProgressHUD class], nil];
#else
        appearance = [MBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ONTOMBProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MBBarProgressView appearanceWhenContainedIn:[ONTOMBProgressHUD class], nil];
#else
        appearance = [MBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[ONTOMBProgressHUD class]]];
#endif
...
- (void)setMode:(ONTOMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == ONTOMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = ONTOMBProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:(ONTOMBProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    ONTOMBProgressHUDBackgroundStyle style = self.style;
    if (style == ONTOMBProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == ONTOMBProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation ONTOMBProgressHUDRoundedButton
